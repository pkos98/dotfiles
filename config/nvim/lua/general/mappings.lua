...
local function init_lsp_mappings(buf_set_keymap, opts)
  ...
  buf_set_keymap('n','<leader>ff', '<cmd>lua vim.lsp.buf.formatting()<CR>', opts, "LSP", "lsp_format", "Format file")
  buf_set_keymap('n', '<leader>e', '<cmd>lua vim.lsp.diagnostic.show_line_diagnostics()<CR>',  { noremap=true, silent=true }, "LSP", "lsp_show_diag", "Show current line diagnostics")
end
...
mapper.map('n', '\x01n', [[:Telescope buffers<CR>]], {}, "Telescope", "telescope_tabs", "List tabs")
...
mapper.map('n', '<A-1>', [[:lua require 'telescope.builtin'.file_browser{cwd = require'telescope.utils'.buffer_dir()}<CR>]], { noremap = true, silent = false }, "Telescope", "telescope_explorer_gui", "File explorer")
...
mapper.map('n', ',,', ":lua require'telescope.builtin'.buffers{}<CR>", opts, "Tab", "find_tab", "Find tab")
mapper.map('n', '<C-w>w', ":windo bd<CR>", opts, "General", "close_tab", "Close current tab")
mapper.map('n', '<C-w>o', ":bufdo bd<CR>", opts, "General", "close_other_tabs", "Close other tabs")
mapper.map('n', '<C-w>a', ":bufdo bwipeout<CR>", opts, "General", "close_all_tabs", "Close all tabs")
