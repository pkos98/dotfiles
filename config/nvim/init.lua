...
require("lazy").setup({
...

  {
    ...
    keys = {
      ...
      { "<leader>lf", function() vim.lsp.buf.format() end, mode = "x" },
    },
    opts = {
      timemout_ms = 500,
      lsp_fallback = false,
      formatters_by_ft = {
        lua = { "stylua" },
        python = { "isort", "ruff" }, -- runs formatters sequentially
        bash = { "shfmt" },
        sh = { "shfmt" },
        zsh = { "shfmt" },
      },
...

  {
    ...
    keys = {
      { "<C-S-+>",    "zo",                                                              noremap = false },
      { "<C-S-_>",    "za",                                                              noremap = false },
...
for _, lsp in pairs({ "terraformls", "pylsp", "clangd", "gopls", "bashls", "rust_analyzer", "awk_ls", "dockerls" }) do
